...
export default function Home() {
  const ceremonyTimestamp = moment("01/08/2025 10:00 AM", "DD/MM/YYYY hh:mm A");
  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-center'>
      <main className="flex flex-col gap-8 items-center sm:items-start container mx-auto my-10">
        ...
        {/* Contador */}
        {ceremonyTimestamp.isSameOrAfter(moment())
          ? (
            <Countdown
        ...
        <footer className='mx-auto'>
          <p className='text-center text-xs text-yellow-800'>
            Hecho con ❤️ por
            {' '}
            <a href='https://instagram.com/wuggycakes' className='border-b-1 border-b-yellow-500 hover:border-b-3 text-yellow-950 font-bold transition-all' target='_blank' rel='nofollow noopener noreferer'>Omar Ureña</a>,
            {' '}
            {ceremonyTimestamp.isSameOrAfter(moment()) ? 'Futuro' : ''} médico Pre-Intero.
          </p>
        </footer>
